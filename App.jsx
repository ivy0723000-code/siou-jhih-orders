...
function cartAmt(cart,customItems,discount){
  ...
  const subtotal=mAmt+cAmt;
  return discount?Math.round(subtotal*(1-Number(discount)/100)):subtotal;
}
...
function newForm(date){
  return{id:"",customerName:"",phone:"",posNo:"",cart:emptyCart(),
    ...
    isKeyed:false,isDelivered:false,driverMatched:false,isSent:false,discount:0,shippingFee:0};
}
...
const DB_ANNOUNCEMENTS= "siou-jhih/announcements";
...
export default function App() {
  ...
  const [mainTab,  setMainTab]  = useState("orders"); // orders | history | customers | announcements
  const [announcements, setAnnouncements] = useState([]);
  const [annForm, setAnnForm] = useState({title:"",body:"",imageUrl:"",startDate:today(),endDate:today(30),active:true});
  ...
  useEffect(() => {
    ...
    const unsub4 = onValue(ref(db, DB_ANNOUNCEMENTS),s => { const d=s.val(); setAnnouncements(d?Object.values(d):[]); });
    setSyncState("synced");
  ...
  useEffect(() => {
    ...
    const newOrds = orders.filter(o=>!prevOrderIds.current.has(o.id));
    if (newOrds.length > 0) {
      const o = newOrds[0];
      setNewOrderAlert({name:o.customerName, type:o.type==="delivery"?"外送":"自取", time:o.deliveryTime});
      // 播放提示音
      try {
        ...
        [0,150,300].forEach(delay => {
          ...
          osc.frequency.value = delay===0?880:delay===150?1100:1320;
          osc.type = "sine";
      ...
      // 瀏覽器通知（iOS Safari 不支援，需保護）
      try {
        if (typeof Notification !== "undefined") {
          if (Notification.permission==="granted") {
            new Notification("🔔 新訂單！", {body:`${o.customerName} · ${o.type==="delivery"?"外送":"自取"} · ${o.deliveryTime}`});
  ...
  async function parseAIOrder() {
    ...
    try {
      const menuList = MENU.map(m=>`id${m.id}:${m.name}(${m.price}元)`).join("\n");
      const prompt = `你是一個訂單解析助手。根據以下菜單和客人訊息，解析出訂單資料，只回傳JSON，不要其他文字。
...
請解析並回傳以下格式的JSON（沒有的欄位填空字串或0）：
{
  "customerName": "姓名",
  "phone": "電話",
  "address": "地址",
  "deliveryTime": "HH:MM格式，沒有就填12:00",
  "deliveryDate": "YYYY-MM-DD格式，明天日期若沒提到",
  "type": "delivery或pickup",
  "note": "整體備註",
  "cart": [
    {"menuId": 數字, "qty": 數量, "note": "備註如半飯/少飯等"}
  ]
}
只回傳JSON，不要markdown格式。`;
      ...
      // Build cart
      const newCart = emptyCart().map(c => {
        const found = (parsed.cart||[]).find(p=>p.menuId===c.menuId);
        return found ? {...c, qty:found.qty||0, note:found.note||""} : c;
      });

      setForm(f=>({
        ...newForm(parsed.deliveryDate||today(1)),
        customerName: parsed.customerName||"",
        phone: parsed.phone||"",
        address: parsed.address||"",
        deliveryTime: parsed.deliveryTime||"12:00",
        deliveryDate: parsed.deliveryDate||today(1),
        type: parsed.type||"delivery",
        note: parsed.note||"",
        cart: newCart,
      }));
      setAddrVal(parsed.address||"");
      setShowAI(false);
      setAiText("");
      setShowForm(true);
    } catch(e) {
      setAiError("解析失敗，請確認格式或手動填寫。錯誤："+e.message);
    }
    setAiLoading(false);
  }
  ...
  function saveAnnouncement() {
    if (!annForm.title.trim()) { alert("請填寫公告標題"); return; }
    const id = editAnnId || "ANN-"+String(Date.now()).slice(-6);
    const newAnn = {...annForm, id};
    const newAnns = editAnnId
      ? announcements.map(a=>a.id===editAnnId?newAnn:a)
      : [...announcements, newAnn];
    setAnnouncements(newAnns);
    const toObj = arr => { const o={}; arr.forEach(i=>{ if(i?.id) o[i.id]=i; }); return o; };
    set(ref(db, DB_ANNOUNCEMENTS), toObj(newAnns));
    setShowAnnForm(false);
    setAnnForm({title:"",body:"",startDate:today(),endDate:today(30),active:true});
    setEditAnnId(null);
  }
  ...
  function openEditAnn(a) {
    setAnnForm({title:a.title,body:a.body,imageUrl:a.imageUrl||"",startDate:a.startDate,endDate:a.endDate,active:a.active});
    setEditAnnId(a.id);
    setShowAnnForm(true);
  }
  ...
  const stats={
    ...
    amount:filtered.reduce((s,o)=>{const b=cartAmt(o.cart,o.customItems);const d=o.discount?Math.round(b*(1-Number(o.discount)/100)):b;return s+d+Number(o.shippingFee||0);},0),
    unpaid:filtered.filter(o=>o.payStatus==="unpaid").length,
  };
  const activeItems=form.cart.filter(c=>c.qty>0);
  const formSubtotal=cartAmt(form.cart,form.customItems);
  const formDiscounted=form.discount?Math.round(formSubtotal*(1-Number(form.discount)/100)):formSubtotal;
  const formTotal=formDiscounted+Number(form.shippingFee||0);
  ...
  function OrderCard({order, showArchive=true}) {
    ...
    const baseAmt=cartAmt(order.cart,order.customItems);
    ...
    return(
      <div className="card">
        ...
        <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6,flexShrink:0,minWidth:92}}>
          ...
          {order.discount>0&&<div style={{fontSize:10,color:"#6ee7b7",background:"rgba(110,231,183,0.1)",border:"1px solid rgba(110,231,183,0.25)",borderRadius:4,padding:"1px 5px",textAlign:"center"}}>{order.discount}% OFF</div>}
          {Number(order.shippingFee)>0&&<div style={{fontSize:10,color:"#93c5fd",background:"rgba(147,197,253,0.1)",border:"1px solid rgba(147,197,253,0.25)",borderRadius:4,padding:"1px 5px",textAlign:"center"}}>運費 ${order.shippingFee}</div>}
  ...
  return (
      ...
      <header style={{background:`linear-gradient(180deg,${B.dark},${B.navy})`,borderBottom:`1px solid rgba(200,169,110,0.2)`,padding:"0 22px",position:"relative",overflow:"hidden"}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"16px 0",position:"relative"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:12}}>
            ...
            <button className="bgh" style={{fontSize:12,padding:"8px 14px",borderColor:"rgba(147,197,253,0.3)",color:"#93c5fd"}} onClick={()=>setShowAI(true)}>🤖 貼訊息填單</button>
            <button className="bgold" onClick={openNew}>＋ 新增訂單</button>
          </div>
        </div>
        {/* Main nav tabs */}
        <div style={{display:"flex",gap:0,borderTop:`1px solid rgba(200,169,110,0.12)`}}>
          {[["orders",`📋 訂單 (${orders.length})`],["history",`📁 歷史 (${history.length})`],["customers",`👥 客戶 (${Object.keys(customers).length})`],["announcements",`📢 公告 (${announcements.length})`]].map(([k,l])=>(
    ...
    const amt=discAmt+Number(order.shippingFee||0);
      ...
      {mainTab==="announcements"&&(
        <div style={{padding:"16px 20px",maxWidth:980,margin:"0 auto"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
            <span style={{fontSize:12,color:B.muted}}>公告會即時顯示在顧客查詢頁面</span>
            <button className="bgold" style={{fontSize:12,padding:"7px 16px"}} onClick={()=>{setAnnForm({title:"",body:"",imageUrl:"",startDate:today(),endDate:today(30),active:true});setEditAnnId(null);setShowAnnForm(true);}}>＋ 新增公告</button>
          </div>
          {announcements.length===0?(
            <div style={{textAlign:"center",padding:"50px 0"}}>
              <div style={{fontSize:32,marginBottom:10,opacity:0.3}}>📢</div>
              <div style={{fontFamily:"'Noto Serif TC',serif",color:B.muted,letterSpacing:2}}>尚無公告</div>
            </div>
          ):announcements.sort((a,b)=>b.startDate.localeCompare(a.startDate)).map(a=>{
            const isActive = a.active && a.startDate<=today() && a.endDate>=today();
            return(
              <div key={a.id} style={{background:B.mid,border:`1px solid ${isActive?"rgba(200,169,110,0.35)":"rgba(138,143,168,0.2)"}`,borderRadius:12,padding:"16px 20px",marginBottom:10,opacity:isActive?1:0.55}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:10,flexWrap:"wrap"}}>
                  <div style={{flex:1}}>
                    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:5}}>
                      <span style={{fontFamily:"'Noto Serif TC',serif",fontSize:15,color:B.white,fontWeight:600}}>{a.title}</span>
                      <span style={{fontSize:10,borderRadius:4,padding:"2px 7px",border:"1px solid",color:isActive?"#6ee7b7":"#8a8fa8",borderColor:isActive?"rgba(110,231,183,0.3)":"rgba(138,143,168,0.3)",background:isActive?"rgba(110,231,183,0.08)":"transparent"}}>
                        {isActive?"● 顯示中":"○ 未顯示"}
                      </span>
                    </div>
                    <div style={{fontSize:12,color:B.muted,marginBottom:6}}>{a.body}</div>
                    <div style={{fontSize:11,color:B.goldD}}>📅 {a.startDate} ～ {a.endDate}</div>
                  </div>
                  <div style={{display:"flex",gap:6,flexShrink:0}}>
                    <button className="bgh" onClick={()=>openEditAnn(a)}>編輯</button>
                    <button className="bgh del" onClick={()=>deleteAnnouncement(a.id)}>刪除</button>
                  </div>
                </div>
              </div>
            );
      ...
      {showAnnForm&&(
        <div className="ovl">
          <div className="modal wide">
            ...
            <div style={{fontFamily:"'Noto Serif TC',serif",fontSize:17,color:B.gold,letterSpacing:2,marginBottom:5}}>{editAnnId?"編輯公告":"新增公告"}</div>
            <div className="gline" style={{marginBottom:18,opacity:0.38}}/>

            {/* 標題 */}
            <div style={{marginBottom:12}}>
              <label className="flbl">公告標題</label>
              <input className="inp" value={annForm.title} placeholder="例：新品上市、假日限定特惠…" onChange={e=>setAnnForm(f=>({...f,title:e.target.value}))}/>
            </div>

            {/* 圖片網址 */}
            <div style={{marginBottom:12}}>
              <label className="flbl">EDM 圖片網址（選填）</label>
              <input className="inp" value={annForm.imageUrl} placeholder="貼上圖片連結，例如 Google 雲端硬碟或 Imgur 的圖片網址…"
                onChange={e=>setAnnForm(f=>({...f,imageUrl:e.target.value}))}/>
              {/* 圖片預覽 */}
              {annForm.imageUrl&&(
                <div style={{marginTop:10,borderRadius:10,overflow:"hidden",border:"1px solid rgba(200,169,110,0.2)",background:"rgba(0,0,0,0.2)",maxHeight:300,display:"flex",alignItems:"center",justifyContent:"center"}}>
                  <img src={annForm.imageUrl} alt="預覽"
                    style={{width:"100%",maxHeight:300,objectFit:"contain",display:"block"}}
                    onError={e=>{e.target.style.display="none";e.target.nextSibling.style.display="flex";}}
                  />
                  <div style={{display:"none",padding:"20px",color:"#f87171",fontSize:12,alignItems:"center",gap:6}}>
                    ⚠ 圖片無法載入，請確認網址是否正確
                  </div>
                </div>
              )}
              <div style={{fontSize:11,color:"#8a6f3e",marginTop:6,lineHeight:1.7}}>
                💡 如何取得圖片網址：上傳到 <strong style={{color:"#c8a96e"}}>Imgur.com</strong>（免費圖床）→ 右鍵複製圖片網址，或 Google 雲端硬碟 → 設定共用 → 複製連結後改成直接連結格式
              </div>
            </div>

            {/* 內文 */}
            <div style={{marginBottom:12}}>
              <label className="flbl">公告內文</label>
              <textarea className="inp" value={annForm.body} placeholder="活動詳情、注意事項、優惠說明…&#10;可以換行輸入" rows={5} style={{resize:"vertical",lineHeight:1.8}} onChange={e=>setAnnForm(f=>({...f,body:e.target.value}))}/>
            </div>

            {/* 日期 */}
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:11,marginBottom:12}}>
              <div>
                <label className="flbl">開始日期</label>
                <input className="inp" type="date" value={annForm.startDate} onChange={e=>setAnnForm(f=>({...f,startDate:e.target.value}))}/>
              </div>
              <div>
                <label className="flbl">結束日期</label>
                <input className="inp" type="date" value={annForm.endDate} onChange={e=>setAnnForm(f=>({...f,endDate:e.target.value}))}/>
              </div>
            </div>

            {/* 狀態 */}
            <div style={{marginBottom:18}}>
              <label className="flbl">狀態</label>
              <div style={{display:"flex",gap:8}}>
                {[[true,"✓ 啟用（顯示給顧客）"],[false,"✗ 停用（隱藏）"]].map(([v,l])=>(
                  <button key={String(v)} onClick={()=>setAnnForm(f=>({...f,active:v}))}
                    style={{flex:1,background:annForm.active===v?"rgba(200,169,110,0.14)":"transparent",color:annForm.active===v?B.gold:B.muted,border:`1px solid ${annForm.active===v?B.goldD:"rgba(200,169,110,0.18)"}`,borderRadius:7,padding:"9px",fontSize:12,cursor:"pointer",fontFamily:"'Noto Sans TC',sans-serif",transition:"all 0.2s"}}>
                    {l}
                  </button>
                ))}
              </div>
            ...
            {/* 預覽效果 */}
            {(annForm.title||annForm.imageUrl||annForm.body)&&(
              <div style={{marginBottom:16,background:"rgba(0,0,0,0.2)",borderRadius:10,padding:"14px 16px",border:"1px solid rgba(200,169,110,0.12)"}}>
                <div style={{fontSize:10,color:"#8a6f3e",letterSpacing:2,marginBottom:10}}>顧客看到的樣子預覽</div>
                {annForm.imageUrl&&<img src={annForm.imageUrl} alt="" style={{width:"100%",borderRadius:8,marginBottom:10,objectFit:"cover",maxHeight:180}} onError={e=>e.target.style.display="none"}/>}
                {annForm.title&&<div style={{fontFamily:"'Noto Serif TC',serif",fontSize:15,color:"#e2c98a",fontWeight:600,marginBottom:6}}>{annForm.title}</div>}
                {annForm.body&&<div style={{fontSize:12,color:"#a89880",lineHeight:1.8,whiteSpace:"pre-wrap"}}>{annForm.body}</div>}
              </div>
            )}

            <div className="gline" style={{marginBottom:13,opacity:0.28}}/>
            <div style={{display:"flex",gap:9}}>
              <button onClick={()=>{setShowAnnForm(false);setEditAnnId(null);}} className="bgh" style={{flex:1,padding:"10px"}}>取　消</button>
              <button onClick={saveAnnouncement} className="bgold" style={{flex:2,padding:"10px"}}>{editAnnId?"儲存修改":"發布公告"}</button>
            </div>
          </div>
        </div>
      )}
      ...
      {showForm&&(
        <div className="ovl">
          <div className="modal">
            ...
            {/* Discount */}
            <div style={{marginBottom:13}}>
              <label className="flbl">折扣優惠（%）</label>
              <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
                {[0,5,10,15,20].map(d=>(
                  <button key={d} onClick={()=>setForm(f=>({...f,discount:d}))}
                    style={{flex:1,minWidth:48,background:form.discount===d?"rgba(110,231,183,0.14)":"transparent",color:form.discount===d?"#6ee7b7":B.muted,border:`1px solid ${form.discount===d?"rgba(110,231,183,0.4)":"rgba(200,169,110,0.18)"}`,borderRadius:7,padding:"8px 4px",fontSize:13,cursor:"pointer",fontFamily:"'Noto Sans TC',sans-serif",transition:"all 0.2s",fontWeight:form.discount===d?600:400}}>
                    {d===0?"無折扣":`${d}% OFF`}
                  </button>
                ))}
              </div>
              {form.discount>0&&(
                <div style={{marginTop:6,fontSize:12,color:"#6ee7b7",display:"flex",justifyContent:"space-between"}}>
                  <span>原價 NT$ {formSubtotal.toLocaleString()}</span>
                  <span>折扣 -{form.discount}% = NT$ {formDiscounted.toLocaleString()}{Number(form.shippingFee)>0?` + 運費 $${form.shippingFee} = NT$ ${formTotal.toLocaleString()}`:''}</span>
                </div>
              )}
            </div>
            ...
            {/* Shipping Fee */}
            <div style={{marginBottom:13}}>
              <label className="flbl">🚚 運費（客人自付，留空表示免運）</label>
              <input className="inp" type="number" min="0" placeholder="0" value={form.shippingFee||""}
                onChange={e=>setForm(f=>({...f,shippingFee:Number(e.target.value)||0}))}
                style={{background:"#0d1428",color:"#93c5fd",fontWeight:600,maxWidth:160}}/>
            </div>
            ...
            {(activeItems.length>0||(form.customItems||[]).some(c=>c.price))&&(
              <div className="csum" style={{marginBottom:13}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:5}}>
                  <span style={{fontSize:11,color:B.goldD,letterSpacing:1}}>{form.discount>0?"折扣後金額":"訂單小計"}</span>
                  <div style={{textAlign:"right"}}>
                    <span style={{fontFamily:"'Noto Serif TC',serif",fontSize:19,fontWeight:700,color:form.discount>0||Number(form.shippingFee)>0?"#6ee7b7":B.goldL}}>NT$ {formTotal.toLocaleString()}</span>
                    {Number(form.shippingFee)>0&&<div style={{fontSize:11,color:"#93c5fd",marginTop:1}}>含運費 NT$ {Number(form.shippingFee).toLocaleString()}</div>}
                  </div>
                </div>
      ...
      {/* ── 新訂單通知 Toast ──────────────────────────────────────────────── */}
      {newOrderAlert&&(
        <div style={{position:"fixed",top:20,right:20,zIndex:999,background:"linear-gradient(135deg,#1a2145,#1e2a52)",border:"1px solid rgba(200,169,110,0.5)",borderRadius:14,padding:"16px 22px",boxShadow:"0 8px 32px rgba(0,0,0,0.5)",minWidth:280,animation:"slideIn 0.3s ease"}}>
          <style>{`@keyframes slideIn{from{transform:translateX(120%);opacity:0}to{transform:translateX(0);opacity:1}}`}</style>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{fontSize:28}}>🔔</div>
            <div>
              <div style={{fontFamily:"'Noto Serif TC',serif",fontSize:15,color:"#c8a96e",fontWeight:600,marginBottom:2}}>新訂單進來了！</div>
              <div style={{fontSize:13,color:"#e8e2d6"}}>{newOrderAlert.name} · {newOrderAlert.type}</div>
              <div style={{fontSize:12,color:"#8a8fa8",marginTop:1}}>取餐時間 {newOrderAlert.time}</div>
            </div>
            <button onClick={()=>setNewOrderAlert(null)} style={{marginLeft:"auto",background:"transparent",border:"none",color:"#8a8fa8",fontSize:18,cursor:"pointer",padding:"0 4px"}}>✕</button>
          </div>
        </div>
      )}
      ...
      {/* ── AI 貼訊息填單 Modal ───────────────────────────────────────────── */}
      {showAI&&(
        <div className="ovl" onClick={()=>setShowAI(false)}>
          <div className="modal" onClick={e=>e.stopPropagation()} style={{maxWidth:580}}>
            <div className="deco tl"/><div className="deco tr"/>
            <div style={{fontFamily:"'Noto Serif TC',serif",fontSize:17,color:"#93c5fd",letterSpacing:2,marginBottom:5}}>🤖 AI 快速填單</div>
            <div className="gline" style={{marginBottom:14,opacity:0.38}}/>
            <div style={{fontSize:12,color:"#8a8fa8",marginBottom:10,lineHeight:1.7}}>
              把客人從 LINE / Facebook 傳來的訂單訊息直接貼進來，AI 會自動解析並填好表單，你再確認送出即可。
            </div>
            <label className="flbl">客人訊息</label>
            <textarea className="inp" rows={8} value={aiText} placeholder="例如：1號-七味唐辛子黃金炸雞餐盒(半飯) 238元 *1  地址：台南市仁德區義林路122號  陳佩萱、0953320183"
              onChange={e=>setAiText(e.target.value)}
              style={{resize:"vertical",lineHeight:1.7,marginBottom:10}}/>
            {aiError&&<div style={{fontSize:12,color:"#f87171",marginBottom:10,background:"rgba(248,113,113,0.08)",border:"1px solid rgba(248,113,113,0.2)",borderRadius:7,padding:"8px 12px"}}>{aiError}</div>}
            <div style={{display:"flex",gap:9}}>
              <button onClick={()=>{setShowAI(false);setAiText("");setAiError("");}} className="bgh" style={{flex:1,padding:"10px"}}>取消</button>
              <button onClick={parseAIOrder} className="bgold" style={{flex:2,padding:"10px",opacity:aiLoading?0.6:1}} disabled={aiLoading}>
                {aiLoading?"🤖 解析中…":"🤖 解析並填入表單"}
              </button>
            </div>
          </div>
        </div>
