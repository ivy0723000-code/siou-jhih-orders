...
export default function App() {
  ...
  function handleTypeChange(v){
    const existing=orders.filter(o=>o.deliveryDate===form.deliveryDate&&o.type==="pickup"&&o.posNo);
    const nums=existing.map(o=>parseInt(o.posNo)).filter(n=>!isNaN(n));
    setForm(f=>({...f,type:v,posNo:v==="pickup"?String(nums.length?Math.max(...nums)+1:1):""}));
  }
          ...
          <div className="modal">
            ...
            <div style={{marginBottom:13}}>
              ...
              {(form.customItems||[]).map((ci,i)=>(
                <div key={i} style={{display:"flex",gap:6,alignItems:"center",marginBottom:6,background:"rgba(252,211,77,0.04)",border:"1px solid rgba(252,211,77,0.15)",borderRadius:8,padding:"8px 10px"}}>
                  <input className="inp" value={ci.name} placeholder="品名" style={{flex:2,background:"#0d1428"}} onChange={e=>setForm(f=>({...f,customItems:f.customItems.map((c,j)=>j===i?{...c,name:e.target.value}:c)}))}/>
                  <input className="inp" value={ci.qty} type="number" min="1" placeholder="數量" style={{width:58,background:"#0d1428",textAlign:"center"}} onChange={e=>setForm(f=>({...f,customItems:f.customItems.map((c,j)=>j===i?{...c,qty:e.target.value}:c)}))}/>
                  <input className="inp" value={ci.price} type="number" placeholder="單價" style={{width:78,background:"#0d1428",color:"#fcd34d",fontWeight:600}} onChange={e=>setForm(f=>({...f,customItems:f.customItems.map((c,j)=>j===i?{...c,price:e.target.value}:c)}))}/>
                  <input className="inp" value={ci.note} placeholder="備註" style={{flex:1,background:"#0d1428",fontSize:11}} onChange={e=>setForm(f=>({...f,customItems:f.customItems.map((c,j)=>j===i?{...c,note:e.target.value}:c)}))}/>
